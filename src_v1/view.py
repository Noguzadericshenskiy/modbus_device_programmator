...
class UpFrame(Frame):
    ...
    def __init__(self):
        ...
        self.btn_connect = Button(self, text="Подключить", command=self.connect_dev, )
        ...
        self.frame_center_left = CenterFrameLeft()
        CenterFrameRight()
        DownFrame()
    ...
    def connect_dev(self):
        ...
        type_dev = self.combobox_type.get()
        self.conn, dev_info = get_client(port, type_dev)
        if self.conn.connect():
            self.lbl_status.config(text="Подключено", background="#00FF00")
        self.frame_center_left.view_table(self.conn, dev_info)
...
class CenterFrameLeft(LabelFrame):
    ...
    def view_table(self, conn, params):
        self.table = Treeview(
            self,
            show="headings",
            selectmode="browse",
        ...
        for row_info in get_info(conn, params):
            if count % 2 == 0:
